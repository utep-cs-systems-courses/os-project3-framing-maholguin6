...
class Archiver:
    ...
    def fileReceived(self, array):
        nameOffset = 2
        # eight bytes reserved for the length of the size
        sizeOffset = 8
        while array:
            # trim the two bytes of the name length off the file
            nameSize = int(array[:nameOffset].decode()); array = array[nameOffset:]
            # trim the length of the actual size of the file name to be created.
            name = array[:nameSize].decode();            array = array[nameSize:]
            # trimm off the size of the file
            size = int(array[:sizeOffset].decode());     array = array[sizeOffset:]
            # trim the actual file lenght
            info = array[:size]; self.write(name, info); array = array[size:]
